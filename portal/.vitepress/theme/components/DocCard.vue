...
import type { Doc } from '../../../../scripts/generate-docs-index'
...
function getCategoryClass() {
  const hash = hashString(props.doc.category)
...
function getTagClass(index: number) {
  const color = colorStyles[(index + 3) % colorStyles.length] // 偏移3避免与分类颜色重复
  return `inline-block rounded px-2 py-0.5 text-xs font-medium ${color.bg} ${color.text} ${color.darkBg} ${color.darkText}`
}
...
function getCoverStyle() {
  ...
  const imageUrl = props.doc.cover || 
                   defaultImages[props.doc.category] || 
                   defaultImages['计算机基础']
